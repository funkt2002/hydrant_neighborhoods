...
import heapq
import itertools
"""Script to identify and find shortest paths using network distance from a given hydrant to nearest network neighboring hydrants.
...
def create_network_snapped(roads, hydrants, search_radius):
    ...
    for _, road in roads.iterrows():
        vertices.extend(Point(coord) for coord in road.geometry.coords)
    ...
    for _, road in roads.iterrows():
        for i in range(len(road.geometry.coords) - 1):
            start, end = road.geometry.coords[i], road.geometry.coords[i + 1]
...
def find_all_nearest_neighbors(G, hydrants, roads, snapped_coords, coord_id_map):
    ...
    for i in tqdm(range(len(snapped_coords))):
        ...
        for target_node, path_length in lengths.items():
            if target_node in hydrant_nodes and target_node != F_node:
                ...
                if not any(node in hydrant_nodes - {F_node, target_node} for node in path[:-1]):
        ...
        # Map each valid path to the neighborhood of the source hydrant
        neighborhoods[F_node_id] = [coord_id_map.get(path[1], path[1]) for path in valid_paths]
...
def main():
    ...
    shortest_paths, neighborhoods =find_all_nearest_neighbors(G, hydrants, roads, snapped_coords, coord_id_map)
    ...
    output_path = r"C:\Users\funkt\OneDrive\Desktop\connected_network-20231115T233050Z-001\connected_network\outputs\prof_paths.shp"
    output_path1 = r"C:\Users\funkt\OneDrive\Desktop\connected_network-20231115T233050Z-001\connected_network\outputs\prof_neighborhoods.shp"
...
# Save the GeoDataFrame as a shapefile
    gdf.to_file(r"C:\Users\funkt\OneDrive\Desktop\connected_network-20231115T233050Z-001\connected_network\outputs\Gnetwork.shp")
    print("All done")
